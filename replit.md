...
*   **Apple-Inspired Candidate Portal**: Standalone candidate experience at `/candidate-access` (login) and `/candidate/*` (portal). Features:
    ...
    - Data Room: **premium DB-backed executive data room** with 19 Varexia demo documents across 7 German categories (Unternehmen & Strategie, Finanzen & Kapitalmarkt, Retail & Operations, Transformation & Investitionen, Governance Risiko & Compliance, People Kultur & Kommunikation, Presse Markt & externe Signale). Features: category sidebar, full-text search, filtering (category/type/read/unread/important/new), "Empfohlener Einstieg" (recommended entry), "Neu für Sie" (new docs), "Zuletzt geöffnet" (recently viewed), inline textSummary reading, file download via signed URLs, server-side viewed-state tracking (DataRoomDocumentView model), progress bar
