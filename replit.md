...
*   **Apple-Inspired Candidate Portal**: Standalone candidate experience at `/candidate-access` (login) and `/candidate/*` (portal). Features:
    ...
    - Data Room: **premium DB-backed executive data room** with 19 Varexia demo documents across 7 German categories (Unternehmen & Strategie, Finanzen & Kapitalmarkt, Retail & Operations, Transformation & Investitionen, Governance Risiko & Compliance, People Kultur & Kommunikation, Presse Markt & externe Signale). **Apple-inspired folder grid design** — categories shown as visual folder cards with colored icons, staggered fade-in animation; clicking opens category with breadcrumb navigation. Document cards: left color accent stripe, category-matched icon, CONFIDENTIAL banner, gradient badges (Important/New), hover lift+shadow with chevron indicator, type pills (Dokument/Memo/Bericht etc.). Document detail view: rich text parser (headings, bullet lists, metadata blocks, tables, bold, dividers), serif body font (Georgia), sticky frosted-glass breadcrumb, category color accent bar, card-style prev/next navigation. Full-text search across all documents, progress bar, file download via signed URLs, server-side viewed-state tracking
    ...
    - Content source files: `lib/candidate-portal/content.ts`, `lib/candidate-portal/auth.ts`
    - **Data Room Prisma models**: `DataRoomCategory` (7 seeded, workspace-scoped, DE+EN labels), `PortalDocument` (extended with slug, tags, documentType, confidentialityLabel, isImportant, isNew, readingTime, textSummary, visibleFrom/Until, categoryId), `DataRoomDocumentView` (per-candidate server-side viewed tracking)
    - **Data Room PDF Viewer**: Documents rendered as real embedded PDFs via same-origin proxy (`?inline=1` on file route streams PDF bytes server-side). `lib/pdf-generator.ts` shared module generates premium-quality PDFs with pdfkit (Varexia logo, category badges, typed metadata boxes, numbered sections, styled tables, quote highlights, page numbers). Instrumentation auto-generates and uploads PDFs to Object Storage for any data room documents missing `objectPath` on startup — ensures production DB gets PDFs without manual script runs.
    - **Data Room candidate API**: `GET/POST /api/candidate-portal/data-room/categories`, `/documents`, `/documents/[slug]`, `/documents/[slug]/view`, `/documents/[slug]/file` (`?inline=1` for same-origin PDF proxy, `?url=1` for signed URL JSON, bare for redirect/download), `/stats`
    - **Data Room admin API**: `GET/POST /api/w/[workspaceSlug]/admin/data-room/categories`, `/categories/[id]`, `/documents`, `/documents/[id]`, `/documents/[id]/upload`
    - **Admin Data Room UI**: `/w/[workspaceSlug]/admin/data-room` (document management), `/w/[workspaceSlug]/admin/data-room/categories` (category CRUD). Added "Data Room" link to AdminSidebar under Werkzeuge
    - **Static fallback**: `lib/candidate-portal/data-room-content.ts` preserved as reference (not imported by DB-backed pages)
