...
export default function PortalManagementSection({
  ...
  function getScheduleLabel(doc: PortalDoc): { text: string; color: string } {
    if (doc.alwaysAvailable) return { text: "Immer zugänglich", color: "bg-[var(--eds-status-blue-bg)] text-[var(--eds-status-blue)]" };
    const now = new Date();
    if (doc.releaseStart || doc.releaseEnd) {
      ...
      if (afterStart && beforeEnd) {
        if (end) return { text: `Aktiv bis ${end.toLocaleDateString("de-DE", { day: "2-digit", month: "short", hour: "2-digit", minute: "2-digit" })}`, color: "bg-[var(--eds-status-green-bg)] text-[var(--eds-status-green)]" };
        return { text: "Freigegeben (offen)", color: "bg-[var(--eds-status-green-bg)] text-[var(--eds-status-green)]" };
      }
      if (start && now < start) return { text: `Ab ${start.toLocaleDateString("de-DE", { day: "2-digit", month: "short", hour: "2-digit", minute: "2-digit" })}`, color: "bg-[var(--eds-status-amber-bg)] text-[var(--eds-status-amber)]" };
      return { text: "Zeitfenster abgelaufen", color: "bg-[var(--eds-bg-sunken)] text-[var(--eds-text-tertiary)]" };
    }
    if (doc.releaseStatus === "released") return { text: "Freigegeben", color: "bg-[var(--eds-status-green-bg)] text-[var(--eds-status-green)]" };
    return { text: "Gesperrt", color: "bg-[var(--eds-bg-sunken)] text-[var(--eds-text-tertiary)]" };
  }
  ...
  function getSAScheduleLabel(sa: SelfAssessmentItem): { text: string; color: string } {
    if (sa.alwaysAvailable) return { text: "Immer zugänglich", color: "bg-[var(--eds-status-blue-bg)] text-[var(--eds-status-blue)]" };
    const now = new Date();
    ...
    if (sa.releaseStatus === "released") return { text: "Freigegeben", color: "bg-[var(--eds-status-green-bg)] text-[var(--eds-status-green)]" };
    return { text: "Gesperrt", color: "bg-[var(--eds-bg-sunken)] text-[var(--eds-text-tertiary)]" };
  }
    ...
    <div className="space-y-6">
      <div className="bg-white border border-[var(--eds-border)] rounded-xl overflow-hidden">
        ...
        <div className="divide-y divide-slate-100">
          {phases.map((phase, idx) => (
            <div key={phase.id} className="px-5 py-4 flex items-center gap-4" data-testid={`phase-control-${phase.id}`}>
              <div className={`w-10 h-10 rounded-xl flex items-center justify-center shrink-0 ${phase.unlocked ? "bg-[var(--eds-status-green-bg)]" : "bg-[var(--eds-bg-sunken)]"}`}>
                <span className={`text-sm font-bold ${phase.unlocked ? "text-[var(--eds-status-green)]" : "text-[var(--eds-text-disabled)]"}`}>{idx + 1}</span>
              </div>
              <div className="flex-1 min-w-0">
                <p className={`text-sm font-medium ${phase.unlocked ? "text-brand-navy" : "text-[var(--eds-text-secondary)]"}`}>{phase.label}</p>
                <p className="text-xs text-[var(--eds-text-disabled)]">{phaseDescriptions[phase.id] || ""}</p>
              </div>
              <button
          ...
          {phases.length === 0 && (
            <div className="px-5 py-6 text-center text-sm text-[var(--eds-text-disabled)]">Phasen werden geladen...</div>
          )}
        </div>
      </div>
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-[var(--eds-text-tertiary)]">Verwalten Sie die Dokumente und Fragebögen, die Kandidaten im Portal sehen</p>
        </div>
        <button
      ...
      {showUpload && (
        <div className="bg-white border border-[var(--eds-border)] rounded-xl p-6 space-y-4">
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              <label className="block text-xs font-medium text-[var(--eds-text-secondary)] mb-1">Titel *</label>
              <input
                value={uploadTitle}
            ...
            <div>
              <label className="block text-xs font-medium text-[var(--eds-text-secondary)] mb-1">Kategorie</label>
              <select
            ...
            <div>
              <label className="block text-xs font-medium text-[var(--eds-text-secondary)] mb-1">Übungszuordnung</label>
              <select
            ...
            <div>
              <label className="block text-xs font-medium text-[var(--eds-text-secondary)] mb-1">Freigabe</label>
              <div className="space-y-2">
                <label className="flex items-center gap-2 cursor-pointer">
                  ...
                  />
                  <span className="text-sm text-[var(--eds-text-primary)]">Immer zugänglich</span>
                </label>
                {!uploadAlwaysAvailable && (
                  <div className="grid grid-cols-2 gap-2">
                    <div>
                      <label className="block text-[10px] text-[var(--eds-text-tertiary)] mb-0.5">Von</label>
                      <input type="datetime-local" value={uploadReleaseStart} onChange={e => setUploadReleaseStart(e.target.value)}
                        ...
                        className="w-full border border-[var(--eds-border)] rounded-lg px-2 py-1.5 text-xs focus:ring-2 focus:ring-brand-blue/30 focus:border-brand-blue outline-none" />
                    </div>
                    <div>
                      <label className="block text-[10px] text-[var(--eds-text-tertiary)] mb-0.5">Bis</label>
          ...
          <div>
            <label className="block text-xs font-medium text-[var(--eds-text-secondary)] mb-1">Beschreibung</label>
            <textarea
              value={uploadDescription}
              onChange={e => setUploadDescription(e.target.value)}
              data-testid="input-portal-doc-description"
              className="w-full border border-[var(--eds-border)] rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-brand-blue/30 focus:border-brand-blue outline-none resize-none"
              rows={2}
      ...
      {grouped.length === 0 && portalDocs.length === 0 && (
        <div className="bg-white border border-[var(--eds-border)] border-dashed rounded-xl py-12 text-center">
          ...
          <p className="text-sm text-[var(--eds-text-tertiary)]">Noch keine Portal-Dokumente vorhanden.</p>
          <p className="text-xs text-[var(--eds-text-disabled)] mt-1">Fügen Sie Ablaufpläne, Übungsdokumente oder Informationen für Kandidaten hinzu.</p>
        </div>
      )}

      {grouped.map(group => (
        <div key={group.value} className="bg-white border border-[var(--eds-border)] rounded-xl overflow-hidden">
          <div className="px-5 py-3 bg-gradient-to-r from-brand-navy/5 to-brand-blue/5 border-b border-[var(--eds-border)]">
              ...
              return (
                <div key={doc.id} data-testid={`portal-doc-${doc.id}`}>
                  <div className="px-5 py-3 flex items-center gap-4">
                    ...
                    <div className="flex-1 min-w-0">
                      <p className="text-sm font-medium text-[var(--eds-text-primary)]">{doc.title}</p>
                      <div className="flex items-center gap-2 text-xs text-[var(--eds-text-disabled)]">
                        ...
                        {doc.downloadAllowed === false && (
                          <span className="text-[var(--eds-status-amber)]">· Nur Ansicht</span>
                        )}
                      </div>
                    </div>
                    <button
                  ...
                  {editingSchedule === doc.id && (
                    <div className="px-5 pb-4 border-t border-[var(--eds-border)] pt-3">
                      <div className="bg-[var(--eds-bg-sunken)] rounded-lg p-4 space-y-3">
                        ...
                        <label className="flex items-center gap-2 cursor-pointer">
                          <input
                            ...
                            className="w-4 h-4 rounded border-[var(--eds-border-strong)] text-brand-blue focus:ring-brand-blue/30"
                          />
                          <span className="text-sm text-[var(--eds-text-primary)]">Download erlaubt</span>
                          {!scheduleForm.downloadAllowed && (
                            <span className="text-[10px] text-[var(--eds-status-amber)] bg-[var(--eds-status-amber-bg)] px-1.5 py-0.5 rounded">Nur Ansicht</span>
                          )}
                        </label>
                        {!scheduleForm.alwaysAvailable && (
                          <div className="grid grid-cols-2 gap-3">
                            <div>
                              <label className="block text-[10px] font-medium text-[var(--eds-text-tertiary)] mb-0.5">Freigabe von</label>
                              <input type="datetime-local" value={scheduleForm.releaseStart} onChange={e => setScheduleForm(f => ({ ...f, releaseStart: e.target.value }))}
                                ...
                                className="w-full border border-[var(--eds-border)] rounded-lg px-2.5 py-1.5 text-xs focus:ring-2 focus:ring-brand-blue/30 focus:border-brand-blue outline-none bg-white" />
                            </div>
                            <div>
                              <label className="block text-[10px] font-medium text-[var(--eds-text-tertiary)] mb-0.5">Freigabe bis</label>
                        ...
                        <div className="flex items-center gap-2 pt-1">
                          ...
                          <button onClick={() => setEditingSchedule(null)}
                            className="text-xs text-[var(--eds-text-tertiary)] hover:text-[var(--eds-text-primary)]">
                            Abbrechen
                          </button>
                        </div>
                      </div>
      ...
      <div className="border-t border-[var(--eds-border)] pt-6">
        <div className="flex items-center justify-between mb-4">
          <div>
            <h3 className="text-base font-semibold text-brand-navy">Fragebögen & Tests</h3>
            <p className="text-xs text-[var(--eds-text-tertiary)]">Selbsteinschätzungen und Fragebögen für Kandidaten</p>
          </div>
          <button
        ...
        {showCreateSA && (
          <div className="bg-white border border-[var(--eds-border)] rounded-xl p-6 space-y-4 mb-4">
            ...
            <div className="flex gap-3">
              ...
              <button onClick={() => setShowCreateSA(false)} className="text-sm text-[var(--eds-text-tertiary)] hover:text-[var(--eds-text-primary)]">
                Abbrechen
              </button>
            </div>
          </div>
        ...
        {selfAssessments.length === 0 && !showCreateSA && (
          <div className="bg-white border border-[var(--eds-border)] border-dashed rounded-xl py-8 text-center">
            <svg className="w-10 h-10 text-[var(--eds-text-disabled)] mx-auto mb-2" fill="none" viewBox="0 0 24 24" strokeWidth={1} stroke="currentColor">
              <path strokeLinecap="round" strokeLinejoin="round" d="M9 12h3.75M9 15h3.75M9 18h3.75m3 .75H18a2.25 2.25 0 002.25-2.25V6.108c0-1.135-.845-2.098-1.976-2.192a48.424 48.424 0 00-1.123-.08m-5.801 0c-.065.21-.1.433-.1.664 0 .414.336.75.75.75h4.5a.75.75 0 00.75-.75 2.25 2.25 0 00-.1-.664m-5.8 0A2.251 2.251 0 0113.5 2.25H15a2.25 2.25 0 012.15 1.586m-5.8 0c-.376.023-.75.05-1.124.08C9.095 4.01 8.25 4.973 8.25 6.108V8.25m0 0H4.875c-.621 0-1.125.504-1.125 1.125v11.25c0 .621.504 1.125 1.125 1.125h9.75c.621 0 1.125-.504 1.125-1.125V9.375c0-.621-.504-1.125-1.125-1.125H8.25zM6.75 12h.008v.008H6.75V12zm0 3h.008v.008H6.75V15zm0 3h.008v.008H6.75V18z" />
            </svg>
            <p className="text-sm text-[var(--eds-text-tertiary)]">Keine Fragebögen erstellt.</p>
          </div>
        )}
        ...
        {selfAssessments.map(sa => {
          ...
          return (
            <div key={sa.id} className="bg-white border border-[var(--eds-border)] rounded-xl mb-3 overflow-hidden" data-testid={`sa-item-${sa.id}`}>
              <div className="px-5 py-3 flex items-center gap-4">
                ...
                <div className="flex-1 min-w-0">
                  <p className="text-sm font-medium text-[var(--eds-text-primary)]">{sa.title}</p>
                  {sa.description && <p className="text-xs text-[var(--eds-text-disabled)]">{sa.description}</p>}
                ...
                <div className="px-5 pb-4 border-t border-[var(--eds-border)] pt-3">
                  <div className="bg-[var(--eds-bg-sunken)] rounded-lg p-4 space-y-3">
                    ...
                    <div className="flex items-center gap-2 pt-1">
                      ...
                      <button onClick={() => setEditingSASchedule(null)} className="text-xs text-[var(--eds-text-tertiary)] hover:text-[var(--eds-text-primary)]">
                        Abbrechen
                      </button>
                    </div>
                  </div>
