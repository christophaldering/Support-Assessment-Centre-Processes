...
const PRIORITY_BADGES: Record<string, { bg: string; text: string }> = {
  hoch: { bg: "bg-red-50", text: "text-red-700" },
  mittel: { bg: "bg-amber-50", text: "text-amber-700" },
  niedrig: { bg: "bg-green-50", text: "text-green-700" },
};
...
function AILabel() {
  return (
    <div className="flex items-center gap-2 text-xs text-slate-500 bg-slate-50 border border-slate-200 rounded-lg px-3 py-2 mb-4">
      <span className="font-medium text-slate-600">KI-gestützte Analyse</span>
      <span>·</span>
...
function PredictiveTab({ profile }: { profile: PredictiveProfile | undefined }) {
  if (!profile) {
    return (
      <div className="text-center py-16 text-slate-400" data-testid="text-no-predictive">
  ...
  return (
    <div data-testid="section-predictive">
      ...
      <div className="flex items-center gap-3 mb-6">
        <h2 className="text-lg font-semibold text-slate-900">Prädiktives Profil</h2>
        <ConfidenceBadge score={profile.confidenceScore} />
        <span className="text-xs text-slate-400">Evidenzabdeckung: {Math.round(profile.evidenceCoverage * 100)}%</span>
      </div>
      ...
      <h3 className="text-sm font-semibold text-slate-700 mb-3">Risikoindikatoren</h3>
      <div className="grid grid-cols-1 md:grid-cols-5 gap-3 mb-8">
        {risks.map((risk, i) => (
          <div key={i} className="border border-slate-200 rounded-lg p-4" data-testid={`card-risk-${risk.category}`}>
            <div className="flex items-center justify-between mb-2">
              <span className="text-xs font-medium text-slate-500">{RISK_LABELS[risk.category] || risk.category}</span>
              <span
            ...
            <p className="text-xs text-slate-600 mb-2">{risk.explanation}</p>
            {risk.contributingFactors.length > 0 && (
              <div className="flex flex-wrap gap-1">
                {risk.contributingFactors.map((f, j) => (
                  <span key={j} className="text-[10px] bg-slate-100 text-slate-500 px-1.5 py-0.5 rounded">{f}</span>
                ))}
              </div>
            )}
          </div>
      ...
      <h3 className="text-sm font-semibold text-slate-700 mb-3">Szenario-Simulationen</h3>
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        {scenarios.map((s, i) => (
          <div key={i} className="border border-slate-200 rounded-lg p-4" data-testid={`card-scenario-${i}`}>
            <div className="flex items-center justify-between mb-2">
              <h4 className="text-sm font-medium text-slate-800">{s.scenario}</h4>
              <ConfidenceBadge score={s.confidence} />
            </div>
            <p className="text-sm text-slate-600 mb-3">{s.predictedBehavior}</p>
            {s.riskFlags.length > 0 && (
              <div className="mb-2">
                <span className="text-xs font-medium text-red-600">Risikoflaggen:</span>
                <div className="flex flex-wrap gap-1 mt-1">
                  {s.riskFlags.map((f, j) => (
                    <span key={j} className="text-xs bg-red-50 text-red-600 px-2 py-0.5 rounded">{f}</span>
                  ))}
                </div>
              </div>
            )}
            {s.compensatingStrengths.length > 0 && (
              <div>
                <span className="text-xs font-medium text-emerald-600">Kompensatorische Stärken:</span>
                <div className="flex flex-wrap gap-1 mt-1">
                  {s.compensatingStrengths.map((f, j) => (
                    <span key={j} className="text-xs bg-emerald-50 text-emerald-600 px-2 py-0.5 rounded">{f}</span>
                  ))}
                </div>
              </div>
            )}
...
function DevelopmentTab({ blueprint }: { blueprint: DevelopmentBlueprint | undefined }) {
  ...
  if (!blueprint) {
    return (
      <div className="text-center py-16 text-slate-400" data-testid="text-no-development">
  ...
  return (
    <div data-testid="section-development">
      ...
      <div className="flex items-center gap-3 mb-4">
        <h2 className="text-lg font-semibold text-slate-900">Entwicklungsplan</h2>
        <ConfidenceBadge score={blueprint.confidenceScore} />
      </div>

      {blueprint.summary && (
        <div className="bg-slate-50 border border-slate-200 rounded-lg p-4 mb-4">
          <p className="text-sm text-slate-700">{blueprint.summary}</p>
        </div>
      )}
        ...
        {subTabs.map(t => (
          <button
            ...
            className={`px-3 py-1.5 text-xs font-medium rounded-full transition-colors whitespace-nowrap ${
              section === t.key ? "text-white" : "bg-slate-100 text-slate-600 hover:bg-slate-200"
            }`}
      ...
      {section === "90d" && (
        <div className="space-y-3">
          {(blueprint.focusAreas90d || []).map((f: FocusArea, i: number) => (
            <div key={i} className="border border-slate-200 rounded-lg p-4" data-testid={`card-focus-${i}`}>
              <div className="flex items-center gap-2 mb-2">
                <h4 className="text-sm font-medium text-slate-800">{f.area}</h4>
                {f.priority && (
                  <span className={`text-xs px-2 py-0.5 rounded-full ${PRIORITY_BADGES[f.priority]?.bg || "bg-slate-100"} ${PRIORITY_BADGES[f.priority]?.text || "text-slate-600"}`}>
                    {f.priority}
                  </span>
                )}
              </div>
              <div className="grid grid-cols-2 gap-4 text-sm">
                <div>
                  <span className="text-xs text-slate-500 font-medium">Aktuell:</span>
                  <p className="text-slate-600">{f.currentState}</p>
                </div>
                <div>
                  <span className="text-xs text-slate-500 font-medium">Zielverhalten:</span>
                  <p className="text-slate-600">{f.targetBehavior}</p>
                </div>
              </div>
              {f.actions && f.actions.length > 0 && (
                <div className="mt-2">
                  <span className="text-xs text-slate-500 font-medium">Maßnahmen:</span>
      ...
      {section === "6m" && (
        <div className="space-y-3">
          {(blueprint.growthTargets6m || []).map((g: GrowthTarget, i: number) => (
            <div key={i} className="border border-slate-200 rounded-lg p-4" data-testid={`card-growth-${i}`}>
              <h4 className="text-sm font-medium text-slate-800 mb-2">{g.target}</h4>
              <div className="text-sm text-slate-600 mb-2">
                <span className="text-xs text-slate-500 font-medium">Messbarer Indikator: </span>
                {g.measurableShift}
              </div>
              {g.milestones && g.milestones.length > 0 && (
                <div className="mb-2">
                  <span className="text-xs text-slate-500 font-medium">Meilensteine:</span>
                  <div className="flex flex-wrap gap-1 mt-1">
                    {g.milestones.map((m: string, j: number) => (
                      <span key={j} className="text-xs bg-blue-50 text-blue-600 px-2 py-0.5 rounded">{m}</span>
                    ))}
                  </div>
                </div>
              )}
      ...
      {section === "12m" && (
        <div className="space-y-3">
          {(blueprint.positioningGoals12m || []).map((p: PositioningGoal, i: number) => (
            <div key={i} className="border border-slate-200 rounded-lg p-4" data-testid={`card-positioning-${i}`}>
              <h4 className="text-sm font-medium text-slate-800 mb-2">{p.goal}</h4>
              <p className="text-sm text-slate-600 mb-2">{p.strategicRationale}</p>
              {p.successIndicators && p.successIndicators.length > 0 && (
                <div className="flex flex-wrap gap-1">
                  {p.successIndicators.map((s: string, j: number) => (
                    <span key={j} className="text-xs bg-emerald-50 text-emerald-600 px-2 py-0.5 rounded">{s}</span>
                  ))}
                </div>
              )}
            </div>
      ...
      {section === "coaching" && (
        <div className="space-y-3">
          {(blueprint.coachingQuestions || []).map((q: CoachingQuestion, i: number) => (
            <div key={i} className="border border-slate-200 rounded-lg p-4" data-testid={`card-coaching-${i}`}>
              <span className="text-xs font-medium text-slate-500">{q.theme}</span>
              <p className="text-sm font-medium text-slate-800 mt-1 mb-1">"{q.question}"</p>
      ...
      {section === "interventions" && (
        <div className="space-y-3">
          {(blueprint.suggestedInterventions || []).map((iv: Intervention, i: number) => (
            <div key={i} className="border border-slate-200 rounded-lg p-4" data-testid={`card-intervention-${i}`}>
              <div className="flex items-center gap-2 mb-2">
                <span className="text-lg">{INTERVENTION_ICONS[iv.type] || "📌"}</span>
                <h4 className="text-sm font-medium text-slate-800">{iv.title}</h4>
                {iv.priority && (
                  <span className={`text-xs px-2 py-0.5 rounded-full ${PRIORITY_BADGES[iv.priority]?.bg || "bg-slate-100"} ${PRIORITY_BADGES[iv.priority]?.text || "text-slate-600"}`}>
                    {iv.priority}
                  </span>
                )}
              </div>
              <p className="text-sm text-slate-600 mb-1">{iv.description}</p>
              <span className="text-xs text-slate-400">Dauer: {iv.duration}</span>
            </div>
          ))}
        </div>
      )}
      ...
      {section === "risks" && (
        <div className="space-y-3">
          {(blueprint.riskMitigationSteps || []).map((r: RiskMitigation, i: number) => (
            <div key={i} className="border border-slate-200 rounded-lg p-4" data-testid={`card-risk-mitigation-${i}`}>
              <div className="flex items-center gap-2 mb-2">
                <span className="text-xs bg-red-50 text-red-600 px-2 py-0.5 rounded font-medium">{r.risk}</span>
              </div>
              <p className="text-sm text-slate-600 mb-1">{r.mitigationAction}</p>
...
function HypothesesTab({ hypotheses, summary }: { hypotheses: DiagnosticHypothesis[]; summary: string }) {
  if (hypotheses.length === 0) {
    return (
      <div className="text-center py-16 text-slate-400" data-testid="text-no-hypotheses">
  ...
  const strengthColor = (s: string) => s === "stark" ? "text-emerald-600" : s === "mittel" ? "text-amber-600" : "text-slate-500";
  const weightColor = (w: string) => w === "erheblich" ? "text-red-600" : w === "moderat" ? "text-amber-600" : "text-slate-500";

  return (
    <div data-testid="section-hypotheses">
      <AILabel />
      <h2 className="text-lg font-semibold text-slate-900 mb-4">Diagnostische Hypothesen</h2>
      ...
      <div className="space-y-4">
        {hypotheses.map((h, i) => (
          <div key={h.id} className="border border-slate-200 rounded-lg overflow-hidden" data-testid={`card-hypothesis-${i}`}>
            <div className="p-4 border-b border-slate-100 bg-slate-50/50">
              <div className="flex items-center justify-between mb-2">
                <span className="text-xs font-medium text-slate-500">Hypothese {i + 1}</span>
                <div className="flex items-center gap-2">
                  <ConfidenceBadge score={h.confidenceScore} />
                  <span className="text-xs text-slate-400">Evidenz: {Math.round(h.evidenceCoverage * 100)}%</span>
                </div>
              </div>
              <p className="text-sm font-medium text-slate-800">{h.hypothesisText}</p>
              {h.alternativeText && (
                <div className="mt-2 text-sm text-slate-600 italic bg-amber-50/50 px-3 py-1.5 rounded border border-amber-100">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 divide-y md:divide-y-0 md:divide-x divide-slate-100">
              <div className="p-3">
                <h5 className="text-xs font-medium text-emerald-600 mb-2">Unterstützende Evidenz</h5>
                {(h.supportingEvidence || []).map((e: EvidenceRef, j: number) => (
                  <div key={j} className="mb-1.5 text-xs">
                    ...
                    <span className="text-slate-600 ml-1">{e.reference}</span>
                  </div>
                ))}
              </div>
              <div className="p-3">
                <h5 className="text-xs font-medium text-red-600 mb-2">Gegenläufige Evidenz</h5>
              ...
              <div className="p-3">
                <h5 className="text-xs font-medium text-blue-600 mb-2">Validierungsbedarf</h5>
                {(h.requiredValidation || []).map((v: ValidationStep, j: number) => (
                  <div key={j} className="mb-1.5 text-xs">
                    <span className="font-medium text-blue-500">[{v.method}]</span>
                    <p className="text-slate-600">{v.question}</p>
                    <p className="text-slate-400 text-[10px]">{v.rationale}</p>
                  </div>
                ))}
              </div>
            </div>
