...
import StandardLanding from "@/app/components/comp/StandardLanding";
import PasswordInput from "@/app/components/PasswordInput";
import { LanguageProvider, useLanguage } from "@/app/providers/LanguageProvider";
import LanguageToggle from "@/app/components/LanguageToggle";

const AppleLanding = dynamic(() => import("@/app/components/comp/AppleLanding"), { ssr: false });
...
export default function CompLobbyPage() {
  return (
    <LanguageProvider>
      <CompLobbyInner />
    </LanguageProvider>
...
function CompLobbyInner() {
  ...
  useEffect(() => {
    try {
      const saved = localStorage.getItem("comp_landing_view");
  ...
  const toggleView = (mode: ViewMode) => {
    setViewMode(mode);
    try { localStorage.setItem("comp_landing_view", mode); } catch {}
  };

  useEffect(() => {
    const checkAuth = async () => {
      try {
        const gateRes = await fetch("/api/comp-bdp/gate/me", { cache: "no-store" });
      ...
      try {
        ...
        if (meRes.ok) {
          await fetch("/api/comp-bdp/gate/general-login", {
            method: "POST",
  ...
  const handlePersonLogin = async () => {
    ...
    try {
      ...
      const res = await fetch("/api/comp-bdp/gate/person-login", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
      ...
      if (data.ok) {
        window.location.href = data.redirectTo || "/comp-bdp";
      } else {
      ...
      <header className="bg-black text-white sticky top-0 z-50">
        <div className="max-w-7xl mx-auto px-6 lg:px-12 h-16 flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <span className="font-bold text-lg tracking-tight">COMP</span>
              ...
              <div className="space-y-3">
                {getDemoRoles(t).map(role => (
                  <button
                    key={role.code}
                    type="button"
                    data-testid={`comp-role-${role.label.toLowerCase()}`}
              ...
              {selectedRole && (
                <div className="mt-4 space-y-3">
                  <div>
                    ...
                    <PasswordInput
                      data-testid="comp-person-password"
                      value="****"
                      readOnly
