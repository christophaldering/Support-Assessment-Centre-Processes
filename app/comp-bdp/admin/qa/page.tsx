...
export default function BdpQAPage() {
  ...
  const runChecks = async () => {
    ...
    try {
      const sessionsRes = await fetch("/api/comp-bdp/sessions");
      const sessions = await sessionsRes.json();
      check("Sessions loaded", Array.isArray(sessions) && sessions.length >= 1, `${sessions.length} sessions`);

      const criteriaRes = await fetch("/api/comp-bdp/criteria");
      ...
      if (draftSession && criteria.length > 0 && teams.length >= 2) {
        const draftRes = await fetch("/api/comp-bdp/scores", {
          method: "POST",
      ...
      if (closedSession && criteria.length > 0 && teams.length >= 2) {
        const closedRes = await fetch("/api/comp-bdp/scores", {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
      ...
      if (criteria.length > 0 && teams.length >= 2) {
        const invalidSumRes = await fetch("/api/comp-bdp/scores/upsert", {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
      ...
      if (nonReleasedSession) {
        const resultRes = await fetch(`/api/comp-bdp/results?sessionId=${nonReleasedSession.id}`);
      ...
      const tieBreakRes = await fetch("/api/comp-bdp/tie-break", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
      ...
      const upsertValidRes = await fetch("/api/comp-bdp/scores/upsert", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
      ...
      const sponsorRes = await fetch("/api/comp-bdp/sponsor", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
      ...
      if (openSession) {
        const participantsRes = await fetch("/api/comp-bdp/participants");
        const allParticipants = await participantsRes.json();
        if (Array.isArray(allParticipants) && allParticipants.length > 0) {
          const noteRes = await fetch("/api/comp-bdp/notes/upsert", {
            method: "POST",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify({
      ...
      const preReleaseExportRes = await fetch("/api/comp-bdp/export?format=json");
      ...
      const demoExportRes = await fetch("/api/comp-bdp/export?format=json&include_demo=false");
      if (demoExportRes.ok) {
      ...
      check("Bewertung page exists", true, "/comp-bdp/bewertung");
      check("Auswertung page exists", true, "/comp-bdp/auswertung");
      check("Admin guard active", !!user?.isAdmin, `isAdmin: ${user?.isAdmin}`);

      const loginRes = await fetch("/api/comp-bdp/auth/login", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
