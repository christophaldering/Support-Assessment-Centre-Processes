...
<p>im Rahmen des <strong>COMP Business Development Pitch (BDP)</strong> laden wir Sie herzlich zur Beobachtung und Bewertung ein.</p>
...
<p>wir laden Sie herzlich als <strong>Experte</strong> zum <strong>COMP Business Development Pitch (BDP)</strong> ein.</p>
...
<p>wir freuen uns, Sie zum <strong>COMP Business Development Pitch (BDP)</strong> einzuladen.</p>
...
const DEFAULT_SUBJECTS: Record<InviteTab, string> = {
  observers: "COMP BDP – Einladung zur Beobachtung & Bewertung",
  experts: "COMP BDP – Einladung als Experte",
  participants: "COMP BDP – Einladung zur Teilnahme am Business Development Pitch",
};
...
export default function BdpInvitationsPage() {
  ...
  const fetchRecipients = async () => {
    try {
      const res = await fetch("/api/comp-bdp/admin/invitations/recipients");
      if (!res.ok) return;
  ...
  const saveEmails = async () => {
    setSaving(true);
    try {
      const res = await fetch("/api/comp-bdp/admin/invitations/save-emails", {
        method: "POST",
  ...
  const generatePreview = async () => {
    ...
    try {
      const res = await fetch("/api/comp-bdp/admin/invitations/preview", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
