...
export default function BdpAdminPage() {
  ...
  useEffect(() => {
    if (!user?.isAdmin) { router.push("/comp-bdp"); return; }
    refresh();
  }, [user]);

  const refresh = () => {
    fetch("/api/comp-bdp/sessions").then(r => r.json()).then(d => { if (Array.isArray(d)) setSessions(d); }).catch(() => {});
    fetch("/api/comp-bdp/teams").then(r => r.json()).then(d => { if (Array.isArray(d)) setTeams(d); }).catch(() => {});
    fetch("/api/comp-bdp/participants").then(r => r.json()).then(d => { if (Array.isArray(d)) setParticipants(d); }).catch(() => {});
    fetch("/api/comp-bdp/observers").then(r => r.json()).then(d => { if (Array.isArray(d)) setObservers(d); }).catch(() => {});
    fetch("/api/comp-bdp/criteria").then(r => r.json()).then(d => { if (Array.isArray(d)) setCriteria(d); }).catch(() => {});
    fetch("/api/comp-bdp/name-mappings").then(r => r.json()).then(d => { if (Array.isArray(d)) setNameMappings(d); }).catch(() => {});
    fetch("/api/comp-bdp/config").then(r => r.json()).then(setConfig).catch(() => {});
    fetch("/api/comp-bdp/session-teams").then(r => r.json()).then(d => { if (Array.isArray(d)) setSessionTeams(d); }).catch(() => {});
    fetch("/api/comp-bdp/team-participants").then(r => r.json()).then(d => { if (Array.isArray(d)) setTeamParticipants(d); }).catch(() => {});
    fetch("/api/comp-bdp/observer-assignments").then(r => r.json()).then(d => { if (Array.isArray(d)) setObserverAssignments(d); }).catch(() => {});
  };
  ...
  const stateTransition = async (id: string, newState: string) => {
    const res = await fetch("/api/comp-bdp/sessions", { method: "PUT", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ id, state: newState }) });
  ...
  const addSession = async () => {
    if (!newName) return;
    await fetch("/api/comp-bdp/sessions", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ name: newName }) });
    setNewName("");
  ...
  const deleteSession = async (id: string) => {
    await fetch(`/api/comp-bdp/sessions?id=${id}`, { method: "DELETE" });
  ...
  const addTeam = async () => {
    if (!newCode) return;
    const res = await fetch("/api/comp-bdp/teams", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ code: newCode }) });
  ...
  const deleteTeam = async (id: string) => {
    await fetch(`/api/comp-bdp/teams?id=${id}`, { method: "DELETE" });
    flash(t("teamDeleted"));
    refresh();
  };
  ...
  const addParticipant = async () => {
    if (!newCode) return;
    const res = await fetch("/api/comp-bdp/participants", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ code: newCode }) });
    if (!res.ok) { const d = await res.json(); flash(d.error || t("error")); return; }
    setNewCode("");
  ...
  const deleteParticipant = async (id: string) => {
    await fetch(`/api/comp-bdp/participants?id=${id}`, { method: "DELETE" });
  ...
  const addCriterion = async () => {
    if (!newName) return;
    await fetch("/api/comp-bdp/criteria", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ name: newName }) });
    setNewName("");
  ...
  const deleteCriterion = async (id: string) => {
    await fetch(`/api/comp-bdp/criteria?id=${id}`, { method: "DELETE" });
  ...
  const addObserver = async () => {
    if (!newCode) return;
    const res = await fetch("/api/comp-bdp/observers", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ code: newCode, role: newRole }) });
    if (!res.ok) { const d = await res.json(); flash(d.error || t("error")); return; }
    setNewCode("");
  ...
  const assignSessionTeam = async () => {
    ...
    const res = await fetch("/api/comp-bdp/session-teams", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ sessionId: stSessionId, teamId: stTeamId }) });
    if (!res.ok) { const d = await res.json(); flash(d.error || t("error")); return; }
  ...
  const unassignSessionTeam = async (id: string) => {
    await fetch(`/api/comp-bdp/session-teams?id=${id}`, { method: "DELETE" });
  ...
  const assignTeamParticipant = async () => {
    ...
    const res = await fetch("/api/comp-bdp/team-participants", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ teamId: tpTeamId, participantId: tpParticipantId }) });
    if (!res.ok) { const d = await res.json(); flash(d.error || t("error")); return; }
  ...
  const unassignTeamParticipant = async (id: string) => {
    await fetch(`/api/comp-bdp/team-participants?id=${id}`, { method: "DELETE" });
    flash(t("assignmentRemoved"));
    refresh();
  };
  ...
  const assignObserver = async () => {
    ...
    const res = await fetch("/api/comp-bdp/observer-assignments", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ sessionId: oaSessionId, userId: oaUserId, canScoreTeamIds: [] }) });
    if (!res.ok) { const d = await res.json(); flash(d.error || t("error")); return; }
  ...
  const unassignObserver = async (id: string) => {
    await fetch(`/api/comp-bdp/observer-assignments?id=${id}`, { method: "DELETE" });
    flash(t("assignmentRemoved"));
    refresh();
  };
  ...
  const saveNameMapping = async () => {
    ...
    await fetch("/api/comp-bdp/name-mappings", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ entityType: mappingType, entityId: mappingId, realName: mappingRealName }) });
    setMappingRealName("");
  ...
  const deleteNameMapping = async (id: string) => {
    await fetch(`/api/comp-bdp/name-mappings?id=${id}`, { method: "DELETE" });
    flash(t("mappingDeleted"));
    refresh();
  };

  const updateConfig = async (updates: any) => {
    await fetch("/api/comp-bdp/config", { method: "PUT", headers: { "Content-Type": "application/json" }, body: JSON.stringify(updates) });
  ...
  const saveTieBreak = async () => {
    ...
    await fetch("/api/comp-bdp/tie-break", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ sessionId: tieSessionId, winnerTeamId: tieWinnerTeamId, decidedById: tieDecidedById, rationale: tieRationale }) });
    flash(t("tieBreakSaved"));
  };

  const handleExport = (format: string, includeDemo: boolean, named: boolean) => {
    window.open(`/api/comp-bdp/export?format=${format}&include_demo=${includeDemo}&named=${named}`, "_blank");
  };
    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        ...
        <div className="flex gap-2">
          <Link href="/comp-bdp/admin/qa-demo" data-testid="link-qa-demo" className="text-xs bg-yellow-100 px-3 py-1 rounded-full hover:bg-yellow-200 font-medium">QA Demo</Link>
          <Link href="/comp-bdp/admin/qa-lite" data-testid="link-qa-lite" className="text-xs bg-green-100 px-3 py-1 rounded-full hover:bg-green-200">QA-Lite</Link>
          <Link href="/comp-bdp/admin/qa" data-testid="link-qa" className="text-xs bg-gray-100 px-3 py-1 rounded-full hover:bg-gray-200">QA</Link>
        </div>
      </div>
            ...
            <div key={s.id} data-testid={`admin-session-row-${s.id}`} className="p-4 border border-gray-100 rounded-xl">
              ...
              <div className="flex gap-2 flex-wrap">
                ...
                <button data-testid={`button-transparency-${s.id}`} onClick={() => {
                  ...
                  fetch("/api/comp-bdp/sessions", { method: "PUT", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ id: s.id, transparencyMode: newMode }) }).then(() => refresh());
              ...
              <div className="flex items-center gap-2">
                <input
                  ...
                  onChange={async (e) => {
                    ...
                    const res = await fetch("/api/comp-bdp/business-case/upload", { method: "POST", body: fd });
          ...
          <div className="space-y-3">
            <div>
              ...
              <div className="flex gap-2">
                ...
                <button data-testid="button-export-print" onClick={() => window.open("/comp-bdp/export/print", "_blank")} className="flex-1 bg-gray-100 py-2 rounded-lg text-sm hover:bg-gray-200">{t("printView")}</button>
              </div>
            </div>
            <div>
