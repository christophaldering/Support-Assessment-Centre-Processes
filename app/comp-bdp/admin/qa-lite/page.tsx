...
export default function QALitePage() {
  ...
  useEffect(() => {
    if (!user?.isAdmin) { router.push("/comp-bdp"); }
  }, [user]);
  ...
  const runTests = async () => {
    ...
    try {
      const sessRes = await fetch("/api/comp-bdp/sessions");
      ...
      if (sessOk) {
        const createRes = await fetch("/api/comp-bdp/sessions", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ name: "QA-Test-Session" }) });
        ...
        if (createOk) {
          const updateRes = await fetch("/api/comp-bdp/sessions", { method: "PUT", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ id: created.id, name: "QA-Test-Session-Updated" }) });
          ...
          const delRes = await fetch(`/api/comp-bdp/sessions?id=${created.id}`, { method: "DELETE" });
    ...
    try {
      const teamsRes = await fetch("/api/comp-bdp/teams");
      const teamsData = await teamsRes.json();
      const teamsOk = teamsRes.ok && Array.isArray(teamsData);

      const partRes = await fetch("/api/comp-bdp/participants");
      const partData = await partRes.json();
      const partOk = partRes.ok && Array.isArray(partData);

      if (teamsOk && partOk && teamsData.length > 0 && partData.length > 0) {
        const tpRes = await fetch("/api/comp-bdp/team-participants", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ teamId: teamsData[0].id, participantId: partData[0].id }) });

        if (tpRes.ok) {
          const tp = await tpRes.json();
          r.push({ name: "TeamParticipants ASSIGN", status: "PASS", detail: `${teamsData[0].code} ← ${partData[0].code}` });

          const verifyRes = await fetch("/api/comp-bdp/team-participants");
          ...
          await fetch(`/api/comp-bdp/team-participants?id=${tp.id}`, { method: "DELETE" });
    ...
    try {
      ...
      if (Array.isArray(sessData) && Array.isArray(teamsData) && sessData.length > 0 && teamsData.length > 0) {
        const stRes = await fetch("/api/comp-bdp/session-teams", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ sessionId: sessData[0].id, teamId: teamsData[0].id }) });

        if (stRes.ok) {
          const st = await stRes.json();
          r.push({ name: "SessionTeams ASSIGN", status: "PASS", detail: `${sessData[0].name} ← ${teamsData[0].code}` });

          const verifyRes = await fetch("/api/comp-bdp/session-teams");
          const verifyData = await verifyRes.json();
          ...
          await fetch(`/api/comp-bdp/session-teams?id=${st.id}`, { method: "DELETE" });
    ...
    try {
      const nmRes = await fetch("/api/comp-bdp/name-mappings", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ entityType: "team", entityId: "qa-test-id", realName: "QA Test Name" }) });
      const nm = await nmRes.json();
      r.push({ name: "NameMappings SAVE", status: nmRes.ok ? "PASS" : "FAIL", detail: nmRes.ok ? `ID: ${nm.id}` : "Fehler" });

      if (nmRes.ok) {
        const verifyRes = await fetch("/api/comp-bdp/name-mappings");
        const verifyData = await verifyRes.json();
        ...
        await fetch(`/api/comp-bdp/name-mappings?id=${nm.id}`, { method: "DELETE" });
    ...
    try {
      const guardRes = await fetch("/api/comp-bdp/health");
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        ...
        <Link href="/comp-bdp/admin" data-testid="link-back-admin" className="text-sm text-gray-400 hover:text-gray-600">← Admin</Link>
      </div>
