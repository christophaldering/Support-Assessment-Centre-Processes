...
export default function BdpBewertungPage() {
  ...
  useEffect(() => {
    fetch("/api/comp-bdp/sessions").then(r => r.json()).then(setSessions).catch(() => {});
  }, []);
          ...
          {openSessions.length > 0 && (
            <div>
              ...
              <div className="space-y-3">
                {openSessions.map(s => (
                  <Link key={s.id} href={`/comp-bdp/bewertung/${s.id}`} data-testid={`card-score-session-${s.id}`} className="block bg-white rounded-2xl p-5 shadow-sm hover:shadow-md transition-all border-l-4 border-green-400">
            ...
            <div>
              ...
              <div className="space-y-3">
                {closedSessions.map(s => (
                  <Link key={s.id} href={`/comp-bdp/bewertung/${s.id}`} data-testid={`card-readonly-session-${s.id}`} className={`block bg-white rounded-2xl p-5 shadow-sm ${user?.environment === "demo" ? "hover:shadow-md transition-all border-l-4 border-[#FFD700]" : "opacity-75"}`}>
                    <div className="flex items-center justify-between">
                      <h3 className="font-bold">{s.name}</h3>
                      <span className={`text-xs px-3 py-1 rounded-full font-medium ${stateColor[s.state]}`}>{stateLabel[s.state]}</span>
                    </div>
