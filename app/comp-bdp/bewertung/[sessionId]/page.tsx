...
export default function BdpScoringPage() {
  ...
  useEffect(() => {
    Promise.all([
      fetch("/api/comp-bdp/sessions").then(r => r.json()),
      fetch("/api/comp-bdp/criteria").then(r => r.json()),
      fetch(`/api/comp-bdp/scores?sessionId=${sessionId}`).then(r => r.json()),
      fetch("/api/comp-bdp/participants").then(r => r.json()),
      fetch(`/api/comp-bdp/notes?sessionId=${sessionId}`).then(r => r.json()),
      fetch(`/api/comp-bdp/sponsor?sessionId=${sessionId}`).then(r => r.json()),
  ...
  const handleSubmitScores = async () => {
    ...
    try {
      const res = await fetch("/api/comp-bdp/scores", {
        method: "POST",
  ...
  const handleSaveNote = async (participantId: string, criterionId: string | null, note: string, generalNote: string, contribution?: number, presence?: number) => {
    try {
      await fetch("/api/comp-bdp/notes", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
  ...
  const handleSponsorToggle = async (teamId: string) => {
    ...
    await fetch("/api/comp-bdp/sponsor", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
  ...
  const openBusinessCase = async (teamId: string) => {
    ...
    try {
      const res = await fetch(`/api/comp-bdp/business-case?teamId=${teamId}`);
      const data = await res.json();
  ...
  return (
    <div className="space-y-4">
      ...
      <Link href="/comp-bdp/bewertung" className="text-gray-400 hover:text-black text-sm">{t("back")}</Link>
