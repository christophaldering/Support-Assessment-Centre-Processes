    ...
    (async () => {
      try {
        ...
        for (const s of released) {
          try {
            const res = await fetch(`/api/comp-bdp/results?sessionId=${s.id}`);
        ...
        try {
          const exportRes = await fetch("/api/comp-bdp/export?format=json&include_demo=false");
          if (exportRes.ok) {
