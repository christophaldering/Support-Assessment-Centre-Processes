...
export async function GET(req: NextRequest) {
  ...
  const wsFilter = { workspace: session.workspaceSlug };

  const sessions = await prisma.bdpSession.findMany({
    where: { state: "RELEASED", ...wsFilter, ...envFilter as any },
    include: {
  ...
  const criteria = await prisma.bdpCriterion.findMany({ where: { active: true, ...wsFilter }, orderBy: { sortOrder: "asc" } });
  const notes = await prisma.bdpIndividualNote.findMany({
    where: { ...envFilter as any, sessionId: { in: releasedIds } },
    include: { participant: true, criterion: true, observer: true },
  });
  const nameMappings = named ? await prisma.bdpNameMapping.findMany({ where: wsFilter }) : [];
  ...
  if (format === "csv") {
    ...
    return new NextResponse(lines.join("\n"), {
      headers: {
        ...
        "Content-Disposition": `attachment; filename="comp-bdp-export.csv"`,
      },
    });
  }
