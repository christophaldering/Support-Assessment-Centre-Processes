...
export async function PATCH(req: NextRequest, { params }: RouteContext) {
  ...
  if (template.analysisStatus !== "analyzed") {
    return NextResponse.json(
      { error: "Template muss den Analyse-Status 'analyzed' haben bevor es aktiviert werden kann." },
      { status: 400 }
    );
  }
  ...
  const updated = await prisma.$transaction(async (tx) => {
    const t = await tx.reportTemplate.update({
      ...
      data: {
        ...
        anonymizationConfirmedById: userId,
        anonymizationConfirmedAt: now,
      },
    });
    ...
    await logAudit({
      ...
      userId,
      action: "style_template_activated",
      entityType: "ReportTemplate",
      entityId: params.templateId,
      details: { templateName: template.name, activatedAt: now.toISOString() },
    });
