...
export default function SlideViewer({ slides, teamName }: SlideViewerProps) {
  ...
  return (
    <div className="flex flex-col h-full">
      ...
      <div className="flex-1 overflow-y-auto px-6 py-8 bg-[#f5f5f7]" data-testid="bdp-case-slide">
        ...
        {slide.kpis && slide.kpis.length > 0 && (
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            {slide.kpis.map((kpi, i) => (
              <div key={i} className="bg-white rounded-xl px-4 py-3 border border-[#0071e3]/30 shadow-sm">
      ...
      <div className="flex items-center justify-between px-6 py-4 border-t border-black/10 bg-white">
        ...
        <div className="flex gap-1.5">
          {slides.map((_, i) => (
            <button
              ...
              className={`w-2.5 h-2.5 rounded-full transition-colors ${i === current ? "bg-[#0071e3]" : "bg-black/15 hover:bg-black/25"}`}
            />
          ))}
        </div>
        ...
        <button
          ...
          className="px-4 py-2 rounded-lg text-sm font-medium bg-[#0071e3] text-black hover:bg-[#005bb5] disabled:opacity-30 disabled:cursor-not-allowed transition-colors"
        >
          Weiter →
        </button>
      </div>
