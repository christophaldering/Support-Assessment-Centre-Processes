...
export default function BdpInvitationsPage() {
  ...
  const fetchRecipients = async () => {
    try {
      const res = await fetch("/api/arag-bdp/admin/invitations/recipients");
      if (!res.ok) return;
  ...
  const saveEmails = async () => {
    setSaving(true);
    try {
      const res = await fetch("/api/arag-bdp/admin/invitations/save-emails", {
        method: "POST",
  ...
  const generatePreview = async () => {
    ...
    try {
      const res = await fetch("/api/arag-bdp/admin/invitations/preview", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
